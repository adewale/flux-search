...
- [ ] **Corpus tests don't run in CI** — `corpus-crud.test.ts` and `corpus-survival.test.ts` read from `data/raw/` (15MB of HTML not in git). CI only runs unit tests. Either add `data/raw/` to the repo (large), use a small representative sample, or run corpus tests as a separate local-only step.
- [ ] **No relevance evaluation harness** — a hand-labeled set of `{query, expected_top_results}` would catch ranking regressions. Both `docs/search.md` and `docs/lessons-learned.md` call for this. Still not built.
- [ ] **No screenshot regression tests in CI** — Playwright is installed but screenshots are taken manually. Could add visual regression tests that run against the deployed site after each deploy.
